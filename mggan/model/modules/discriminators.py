...
class MultiDiscriminatorTrajectory(nn.Module):
    def __init__(
        ...
        if scene_dim > 0:
            self.scene_encoder = AttentionGlobal(
                noise_attention_dim=0, PhysFeature=True, 
                num_layers=2, channels_cnn=8
            )
            h_dim += scene_dim
    ...
    def encode(self, in_xy, in_dxdy, pred_xy, pred_dxdy, mask=None):
        ...
        pred_enc = self.pred_encoder(
            pred_encoder_inp.reshape(n_samples * b, -1))
        ...
        for d_hist, d in zip(self.discs_hist, self.discs):
            ...
            for name, tensor in d_hist.named_parameters():
                if "weight" in name or "bias" in name:
                    tensor.data = (
                        tensor.data * (1 - alpha) +
                        name_weight_mapping[name] * alpha
                    )
