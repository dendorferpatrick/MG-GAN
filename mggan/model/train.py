...
from mggan.visualization import plot_scene
import io
...
class PiNetMultiGeneratorGAN(MultiGeneratorGAN):
    ...
    def get_predictions_batch(self, batch, num_preds=20, strategy="sampling", vis = False):
        ...
        return {"out_xy": preds, "out_dxdy": preds_rel, 
        "probs": probs, "gen_idxs": gen_idxs}

    def get_predictions(self, loader, num_preds=20, strategy="sampling", vis = False):
        # print(loader.sampler)
        # assert isinstance(loader.sampler, torch.utils.data.SequentialSampler)
        ...
        for i, batch in tqdm(enumerate(loader), total=len(loader)):
            ...
            all_gt.append(to_numpy(gt_xy))
            if len(all_seq_start_end) > 0:
                last_max = all_seq_start_end[-1][1]
            else: last_max = 0 
            all_seq_start_end.extend([[start + last_max, end + last_max] for (start, end) in seq_start_end])

        # if vis:
        #     [start, end] =  batch["seq_start_end"][0]
            
        #     plot_scene(
        #     in_xy=in_xy[:, start : end].permute(1, 0, 2).cpu(), gt_xy=None, pred_xy=preds[:, :, start:end].permute(2, 1, 0, 3).cpu(), img=batch["scene_img"][0]["scaled_image"], scale=0.25, ax=None, lw=3
        #     )
        #     buf = io.BytesIO()
        ...
        #     self.writer.log( {'trajectory_jpeg': image})

        return np.concatenate(all_preds, 2), np.concatenate(all_gt, 1), all_seq_start_end
    ...
    def check_accuracy(
        ...
        **kwargs,
        ...
        preds, gt_traj, seq_start_end = self.get_predictions(loader, num_preds=num_k, strategy=predict_strategy, vis = True)
        metrics = evaluate_ade_fde(loader.dataset, preds, gt_traj, seq_start_end, [num_k])
        return metrics
